...
func secondMarkers(_ ctx:CGContext, _ x:CGFloat, _ y:CGFloat, _ radius:CGFloat, _ sides:Int, _ color:UIColor) {
    // retrieve points
...
func drawText(_ rect:CGRect, _ ctx:CGContext, _ x:CGFloat, _ y:CGFloat, _ radius:CGFloat, _ sides:NumberOfNumerals, _ color:UIColor) {
    ...
    for p in points.enumerated() {
        if p.offset > 0 {
            ...
            let attr: Dictionary = [NSAttributedString.Key.font:aFont!,NSAttributedString.Key.foregroundColor:UIColor.white]
            // create the attributed string
            let str = String(p.offset * multiplier)
            guard let text = CFAttributedStringCreate(nil, str as CFString, attr as CFDictionary) else { return }
...
class View: UIView {
        ...
        
    {
        ...
        drawText(rect, ctx!, rect.midX, rect.midY, rad, .twelve, UIColor.white)
